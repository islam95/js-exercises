...
function replace(arr, valueToReplace, newValue) {
  var start = arr.slice(0, arr.indexOf(valueToReplace)); // slice left part
  var newArr = start.concat(newValue);
  var end = arr.slice(arr.indexOf(valueToReplace) + 1, arr.length); // slice right part
